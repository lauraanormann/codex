...
pub(crate) type TablePickerOnSelected<T> = Box<dyn Fn(&AppEventSender, T) + Send + Sync>;
...
impl<T: Clone> SearchableTablePickerView<T> {
    ...
    fn accept(&mut self) {
        ...
        {
            (self.on_selected)(&self.app_event_tx, item.value.clone());
            self.complete = true;
        } else {
            self.complete = true;
        }
    ...
    fn render_search(&self, area: Rect, buf: &mut Buffer) {
        let line = if self.query.is_empty() {
            vec![self.search_placeholder.clone().dim()].into()
        } else {
...
impl<T: Clone + 'static> BottomPaneView for SearchableTablePickerView<T> {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        let hint_reserved = if remaining_height >= 2 { 2 } else { 0 };
        let rows_area = Rect {
            x: area.x,
        ...
        if hint_reserved == 2 {
            let hint_start = area.y.saturating_add(area.height).saturating_sub(2);
            if hint_start >= area.y {
                let blank_area = Rect {
                    x: area.x,
                    y: hint_start,
                    width: area.width,
