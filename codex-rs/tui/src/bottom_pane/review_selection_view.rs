...
impl ReviewSelectionView {
    ...
    fn accept(&mut self) {
        ...
        let render_full_list = self.comments.len() > 1;
        message_lines.push(if render_full_list {
            "Full review comments:".bold().into()
        } else {
            "Review comment:".bold().into()
        });

        for (idx, comment) in self.comments.iter().enumerate() {
            message_lines.push("".into());
            ...
            message_lines.push(format!("- {marker} {title} — {location}").into());
            for body_line in comment.body.lines() {
                message_lines.push(format!("  {body_line}").into());
            }
        }
