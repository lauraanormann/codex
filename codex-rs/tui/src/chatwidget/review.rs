            ...
            if output.findings.is_empty() {
                let mut body_lines: Vec<RtLine<'static>> =
                    if !output.overall_explanation.trim().is_empty() {
                        ...
                        vec![RtLine::from("Review failed -- no response found")]
                    };
                body_lines.insert(0, RtLine::from(""));
                body_lines.insert(0, RtLine::from("Review results:".bold()));
                body_lines.insert(0, RtLine::from(""));
...
impl<'a> ReviewController<'a> {
    ...
    pub(crate) fn open_review_popup(&mut self) {
        ...
        let build_review_actions = |review_request: ReviewRequest| -> Vec<SelectionAction> {
            vec![Box::new(move |_, tx: &AppEventSender| {
        ...
        let actions = build_review_actions(ReviewRequest {
    ...
    pub(crate) fn open_branch_picker(
        ...
        let view = BranchPickerView::new(
            cwd,
            app_event_tx.clone(),
            Box::new(move |tx2, branch: String| {
                let prompt = format!(
